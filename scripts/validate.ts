...
function section(title: string, problems: string[], note?: string) {
  if (problems.length === 0) {
    console.log(
      `${C.green}✓${C.reset} ${C.bold}${title}${C.reset}${note ? ` ${C.dim}${note}${C.reset}` : ''}`,
    );
  } else {
    failed = true;
    console.log(
      `${C.red}✗ ${C.bold}${title}${C.reset} ${C.red}(${problems.length} problema(s))${C.reset}`,
    );
...
// rawR32 já vem tipado pelo zod (união discriminada por `rank`).
const allowed: Partial<Record<GroupId, GroupId[]>> = {};
for (const game of rawR32) {
  for (const side of [game.home, game.away]) {
    if (side.rank === 3) {
      allowed[side.lookupWinner] = side.allowedGroups;
    }
  }
}
...
// ---- 1. mata-mata ----
section(
  'Árvore do mata-mata (R32 → final)',
  validateBracket(structure),
  '32 jogos, folhas e fluxo conferidos',
);
...
section('Anexo C — respeita allowedGroups da FIFA', [
  ...rep.problems,
  ...rep.impossible.map((k) => `combo "${k}" não tem alocação possível`),
]);
...
console.log();
console.log(
  `  ${C.dim}Alocação ÚNICA (provada por lógica):${C.reset} ${C.green}${unique}/${total}${C.reset}`,
);
if (rep.ambiguous.length > 0) {
  console.log(
    `  ${C.yellow}Ambíguos (allowedGroups admite >1 alocação válida): ${rep.ambiguous.length}/${total}${C.reset}`,
  );
  console.log(
    `  ${C.dim}→ Nesses, só um diff com a tabela oficial da FIFA confirma a escolha exata.${C.reset}`,
  );
  console.log(
    `  ${C.dim}  exemplos: ${rep.ambiguous.slice(0, 12).join(', ')}${rep.ambiguous.length > 12 ? ' …' : ''}${C.reset}`,
  );
} else {
  console.log(
    `  ${C.green}Todos os 495 combos têm alocação ÚNICA → a tabela está provada sem precisar da fonte externa.${C.reset}`,
  );
}
