...
export function AdminPanel({ userId }: { userId: string }) {
  ...
  async function save(matchId: string) {
    ...
    if (!error) setDraft((prev) => { const n = { ...prev }; delete n[matchId]; return n; });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-400">
          Cravar resultados <span className="text-emerald-400 font-medium">oficiais</span> dos
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {GROUP_IDS.map((g: GroupId) => {
          ...
          return (
            <section key={g} className="rounded-xl bg-slate-900 ring-1 ring-slate-800 p-3">
              <h2 className="font-bold mb-2">Grupo {g}</h2>
                ...
                {matches.map((m) => {
                  ...
                  return (
                    <div key={m.id} className="flex items-center gap-1.5 text-sm">
                      <span className="text-[10px] text-slate-600 w-4">R{m.round}</span>
                      <span className="flex-1 text-right truncate">{teamName(m.home)}</span>
                      <ScoreBox value={v.h} onChange={(x) => setDraftFor(m.id, { h: x })} label="casa" />
                      <span className="text-slate-600">×</span>
                      <ScoreBox value={v.a} onChange={(x) => setDraftFor(m.id, { a: x })} label="fora" />
                      <span className="flex-1 truncate">{teamName(m.away)}</span>
                      <button
                        onClick={() => save(m.id)}
                        disabled={!canSave}
                        className="text-[11px] px-2 py-0.5 rounded bg-emerald-600 text-white disabled:opacity-40"
                      >
                        {isOfficial ? "↑" : "salvar"}
                      </button>
                      {isOfficial && (
